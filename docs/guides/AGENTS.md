...
3. `snippets/drivers/<category>/<slug>/driver-page.mdx` — intro line, a `## Creating an Instro<Category> with <Driver>` example, and a closing link to the driver's SDK reference anchor. Copy the shape of an existing driver in the same category.
...
7. **Examples** — add script(s) under `examples/<category>/` (or the relevant `packages/instro-*/.../examples/`), run `just gen-examples` to generate the example pages and the category's `index.mdx`, then add one line to `docs.json`'s Examples tab pointing at that index — the one manual edit the generator can't make. Unstable examples (`packages/instro-unstable/instro/unstable/<submodule>/examples/*.py`) are the exception: they all land on the single `examples/unstable/index.mdx`, one heading per submodule, so a new unstable submodule needs no `docs.json` edit.
8. **`docs/sdk/`** (sibling mkdocs site) — add the category to `docs/sdk/mkdocs.yml`'s nav and create `docs/sdk/src/instruments/<category>.md` with `::: instro.<category>` directives, so step 6 of "Adding a new instrument driver" has somewhere to link.
