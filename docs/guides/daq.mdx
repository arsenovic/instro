...
- **DewesoftX** - Live channels from a running DewesoftX instance over DCOM (Windows only, `instro-unstable`)
...
daq.configure_voltage_output(
    physical_channel=CHANNEL_0, alias=f"ao_0", range_min=0, range_max=5
)
daq.configure_voltage_output(
    physical_channel=CHANNEL_1, alias=f"ao_1", range_min=0, range_max=5
)
...
| `configure_ai_hw_sample_rate(sample_rate, samples_per_channel=None)` | Set hardware timing for analog input |
...
The analog methods below back `InstroDAQ`'s [typed channel configuration](#typed-channel-configuration); the support matrix there lists per-driver support. `configure_ai_voltage_channel` is required; the others default to `NotImplementedError` for hardware that lacks the capability.
