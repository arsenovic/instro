...
publishers=[
    ...
    FilePublisher(director='/tmp',format='jsonl')
]
...
Every [`Measurement` and `Command`](library#measurements-and-commands) is now recorded by each [`Publisher`][Publisher-link] in `publishers`. An `Instrument` will `close()` all publishers when it closes. 
...
class Publisher(Protocol):
    ...
    def close(self) -> None:
        ...
        ...
```
* `publish`: returns [`Measurements` and `Commands`](library#measurements-and-commands).
...
See [Publishers: publish shared](/instrumentation/examples/publishers/publish_shared) for a full example.
