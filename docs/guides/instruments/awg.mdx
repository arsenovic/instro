...
- **`autostart`**: When `True`, opens the connection and starts background polling immediately. Requires `config`: `start()` refuses to begin polling until at least one channel has a waveform programmed, and the config's `channels` block is what guarantees that.
...
`driver.name` must match one of the drivers supported by Instro (`RigolDG1022Z`, `Keysight33521B`). The `visa` block accepts every `VisaConfig` field, so the backend, timeout, or serial setting can be set from JSON too.
...
- **`waveform`** (required): one of `sine`, `square`, `sawtooth`, `triangle`, `pulse`, `arbitrary`, or `static_value`, discriminated by a `shape` field. `arbitrary`'s `samples` accepts either an inline array of at least 2 floats normalized to `[-1.0, 1.0]`, or a string path to a CSV file containing them; `sample_rate_sas` is the sample rate in samples per second.
  ...
  A CSV path is used as written. A relative path resolves against the working directory of the process, not the config file, so use an absolute path unless you control where the process starts. The file is read each time the waveform is built, which includes `open()`, so it must still be present then and later edits to it do take effect.
...
```python
# instantiating InstroAWG with a plain dict config
awg = InstroAWG(config={
    "device": {
        "name": "bench_awg",
        "description": "Bench waveform generator",
        "manufacturer": "Rigol",
    ...
    "driver": {
        ...
        "num_channels": 2,
        "connection_type": "visa",
        "visa": {"visa_resource": "USB0::0x1AB1::0x0642::DG1ZA000000000::INSTR"},
    },
    "channels": {
...
# instantiating InstroAWG with a pre-built AWGConfig object
awg = InstroAWG(config=my_awg_config)
```

A passed-in `AWGConfig` is copied up front, so later edits to the object you passed don't reach the AWG.

JSON configs are validated strictly: `AWGConfig` forbids any field not listed above (`version`, `instrument`, `device`, `driver`, `channels`, `timing`, `publishers`), so there is no `**kwargs`-style escape hatch from JSON. Set default tags via the direct Python constructor instead.
