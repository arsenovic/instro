...
- **Keysight**: 34461A Truevolt DMM via SCPI/VISA (`Keysight34461A`)
- **Keithley**: 2400 SourceMeter, measurement-only mode (`Keithley2400`)
- **Simulated**: no hardware required; pairs with the bundled [DMM simulator](/instruments/dmm-simulator) (`SimulatedDMM`)
...
- **`name`**: A name for this DMM instance. Used as a prefix for channel names when publishing. Falls back to `config.device.name` when `config` is given.
- **`driver`**: A concrete `DMMDriverBase` instance (e.g. `Agilent34401A`, `Keithley2400`) configured with the connection details for that model. Mutually exclusive with `config`.
- **`config`**: A `DMMConfig`, a dict, or a path to a JSON config file, as an alternative to `driver`. See [From a JSON Config File](#from-a-json-config-file) below.
- **`publishers`**: Optional list of publishers to attach. Combined with any publishers declared in `config`.
- **`autostart`**: When `True`, opens the connection and starts background polling immediately. Requires `config` with a `measurement` block, since polling cannot start without a measurement function.
...
`driver.name` must match one of the registered vendor/model keys (`Agilent34401A`, `Keithley2400`, `Keysight34461A`, `SimulatedDMM`). The `visa` block accepts every `VisaConfig` field, so a non-default backend, timeout, or serial setting can be set from JSON too.

The optional `measurement` block declares the initial measurement state. It is applied through the public setters (`set_measurement_function`, `set_digits`, ...) when `open()` runs, so it publishes the same `.cmd` channels the equivalent manual calls would:
...
- **`digits`**: resolution in digits. Omit to keep the instrument default. Driver support varies: the Agilent 34401A accepts it, while the Keysight 34461A rejects it in favor of `aperture_nplc` (hence its absence from the example above).
...
An optional top-level `timing` section (`{"poll_interval": 1.0}`) sets the background daemon's polling interval. Because background polling cannot start without a measurement function (`start()` raises without one, matching the runtime API), a config that declares `timing` must also declare `measurement`. Pass `autostart=True` to open the connection and start polling immediately:

```python
dmm = InstroDMM(config="bench_dmm.json", autostart=True)
```

`config` also accepts a plain dict or an already-built `DMMConfig`, so a config built or received elsewhere in code can construct an `InstroDMM` directly, without a JSON round-trip.

JSON configs are validated strictly: `DMMConfig` forbids any field not listed above (`version`, `instrument`, `device`, `driver`, `measurement`, `timing`, `publishers`), so there is no `**kwargs`-style escape hatch from JSON. Set default tags via the direct Python constructor instead.
...
All measurement methods return [`Measurement`](/library/library#measurement) objects. This is common amongst all `Instrument` objects.
...
Every measurement/command call produces a channel keyed under `{name}.{descriptor}`, where `{name}` is the constructor argument and `{descriptor}` is the row below. The `read()` descriptor depends on the active measurement function. Substitute the lowercased function name (e.g. `dc_voltage`, `ac_voltage`, `two_wire_resistance`). DMM had no v1.0 channel-naming change. [`legacy_naming`](/library/library#backwards-compatible-channel-naming-legacy_naming) is accepted but is a no-op for this category.
...
| `InstroDMM(config=..., publishers=None, autostart=False, **kwargs)` | Construct from a `DMMConfig`, a dict, or a path to a JSON config file |
...
<Note>
`set_aperture_seconds` is part of the DMM contract but is not implemented by the bundled drivers (Agilent 34401A, Keysight 34461A, Keithley 2400). Calling it on those drivers raises `NotImplementedError`. Use `set_aperture_nplc` to set integration time on the shipped drivers. See [Exceptions](/library/exceptions) for the distinction between `NotImplementedError` and `FeatureNotSupportedError`.
</Note>
