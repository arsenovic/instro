...
- **`**kwargs`**: Additional keyword arguments become default tags when using a publisher that supports tags (like `NominalCorePublisher`). Pass [`legacy_naming=True`](/library/library#backwards-compatible-channel-naming-legacy_naming) here to publish pre-v1.0 channel names.
...
The three optional initial-state blocks are applied through the public setters when `open()` runs, so they publish the same `.cmd` channels the equivalent manual calls would. The apply order follows what the hardware needs. Within each `channels` entry, `coupling` and `probe_attenuation` apply before `vertical_scale` and `vertical_offset`, because the probe factor rescales the channel's probe-referred scale. The `trigger` block applies next. If `start_acquisition_on_open` is set, `run()` fires here, before the `acquisition` block, because some scopes (Siglent) only accept acquisition-mode changes while running. Within `acquisition`, `average_count` applies before `mode`, then `horizontal_scale`. Every configured measurement is then prepared on the instrument, so vendors that compute measurements during acquisition (Tektronix) have their slots in place before the first poll. If any setter raises, the driver is closed and the error propagates.
...
Some instruments defer settings based on their run state. The Siglent SDS1000X-E only applies acquisition-mode changes while acquiring, so an `acquisition.mode` applied to a stopped Siglent without `start_acquisition_on_open` does not take effect; the read-back after `open()` reports the unchanged mode and the snap warning surfaces it. This matches the runtime `set_acquisition_mode()` behavior on that instrument.

After the blocks apply, `open()` calls `sync_configuration()` and logs one warning per field the instrument reports differently from the config, for example `config requested ch1.vertical_scale=0.3 but instrument reports 0.2`. Floats compare within 1% (`instro.scope.scope.SNAP_REL_TOL`, with `SNAP_ABS_TOL` as the floor near zero), so a read-back rounded to three significant figures stays quiet while any real 1-2-5 step snap still warns. Trigger fields are not checked, since `sync_configuration()` does not read them back.

Config never starts acquisition by default. Set `acquisition.start_acquisition_on_open: true` to call `run()` during `open()`, after the channel and trigger blocks are in place and before the acquisition block applies. Without it, background-polled `measure()` calls return `NaN` until you call `scope.run()` or the operator starts the scope from the front panel, which keeps passive monitoring of an operator-driven scope a first-class workflow. One-shot arming (`single()`) and waveform, screenshot, and settings-file operations stay runtime-only.
...
See [Two ways to get data](/overview/using-instro#two-ways-to-get-data) for more on background fetching.

## Published channels

Every call produces a channel keyed under `{name}.{descriptor}`, where `{name}` is the constructor argument and `{descriptor}` is the row below. Substitute `{N}` with the channel number and `{type}` with the lowercased measurement type (e.g. `vpp`). Pass [`legacy_naming=True`](/library/library#backwards-compatible-channel-naming-legacy_naming) to the constructor for the pre-v1.0 names.
