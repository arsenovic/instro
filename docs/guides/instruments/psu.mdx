...
Every measurement/command call produces a channel keyed under `{name}.{descriptor}`, where `{name}` is the constructor argument and `{descriptor}` is the row below. Substitute `{N}` with the actual channel number (`1`, `2`, …). If you need the pre-v1.0 channel names instead, pass [`legacy_naming=True`](/library/library#backwards-compatible-channel-naming-legacy_naming) to the constructor.
...
| `get_operating_mode(channel=N)` | `ch{N}.operating_mode` | telemetry |
...
| `get_operating_mode(channel=N)` | Query the regulation mode (returns Measurement with value `CV`, `CC`, or `OFF`) |
...
For development and testing without physical hardware, Nominal Instrumentation includes a local SCPI power-supply simulator. See [PSU simulator](/instruments/psu-simulator) for the quickstart, usage notes, and supported command set.
