...
`VisaDriver` addresses GPIB, USB-TMC, TCPIP (SOCKET, VXI-11, HiSLIP), and ASRL (serial) resources. For a comparison against the other transports, see [Transports](/library/transports/overview).

To author an instrument driver that composes `VisaDriver`, see the driver-development section for that category on [Custom Instruments](/library/custom-instruments): [power supplies](/library/custom-instruments#power-supplies), [electronic loads](/library/custom-instruments#electronic-loads), [multimeters](/library/custom-instruments#digital-multimeters), [waveform generators](/library/custom-instruments#arbitrary-waveform-generators), or [DAQ](/library/custom-instruments#data-acquisition). This page is the transport reference those sections link back to.
...
`VisaDriver` follows the [standard transport lifecycle](/library/transports/overview#lifecycle): construct, `open()`, I/O, `close()`, with both `open()` and `close()` idempotent. Two behaviors are specific to VISA:
...
[Locking](/library/transports/overview#atomic-multi-step-sequences) and [shared ownership](/library/transports/overview#shared-ownership) work the same for `VisaDriver` as for any transport.
...
| `open(holder=None)` | Open the resource manager and resource; apply terminator/timeout/serial config. Idempotent. Pass a holder to register shared ownership: returns `True` if `holder` is the first owner. See [Shared ownership](/library/transports/overview#shared-ownership). |
| `close(holder=None)` | Close the resource and resource manager. Idempotent. Bare `close()` declines and logs a warning if another driver still holds the connection. With a holder, removes that owner; the close that empties the owner list tears the connection down. Raises `UnknownHolderError` if an unrecognized holder closes while others still own the connection. See [Shared ownership](/library/transports/overview#shared-ownership). |
